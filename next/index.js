...
async function scrapeText() {
  ...
  await page.goto('https://nikari.fi/products/chairs,linea-collection/akademia-armrest/', { waitUntil: 'networkidle0' });

  // Kerätään kaikki luokan "col-inner" sisällä olevien <p>-elementtien tekstisisällöt
  const textContent = await page.evaluate(() => {
    const texts = Array.from(document.querySelectorAll('.col-inner p'), element => element.innerText);
    return texts;
  });
  ...
  fs.appendFile('nikari-texts.txt', textContent.join('\n\n'), 'utf-8', (err) => {
    if (err) throw err;
