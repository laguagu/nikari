...
export function Chatbot() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const newMessages = [...messages, { text: userInput, sender: "user" as "user" | "assistant" }];
    ...
    try {
      ...
      setMessages(newMessages.concat([{ text: botResponse.response, sender: "assistant" }]));
    } catch (error) {
  ...
  return (
    <div className="grid w-full max-w-6xl grid-cols-1 gap-4 p-4 rounded-lg border border-gray-200 dark:border-gray-800">
